...
fetch(json_url).then(Response => Response.json())
  .then((data) => {
    ...
    document.getElementById('rate').innerHTML = `<span>IMDB</span><i class="bi bi-badge-hd"></i><i class="bi bi-star-fill"></i> ${data[0].imdb};`
    ...
    data.forEach(element => {
        ...
        <img src="${sposter}" alt="">
                <div class="cont">
                  <h3>${name} </h3>
                  <p>${genre}, ${date}, <span>IMDB</span></i><i class="bi bi-star-fill"></i> ${imdb}</p>
                </div>
          `
    ...
    search_input.addEventListener('keyup', () => {
      ...
      for (let index = 0; index < a.length; index++) {
        ...
        if (search_input.value == 0) {
            search.style.visibility = "hideen"
